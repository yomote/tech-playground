...
function DemoCard({ demo, index }: { demo: DemoEntry; index: number }) {
  ...
  return <Card variant="outlined" className="demo-card"><CardActionArea component={Link} to={`/demos/${demo.id}`} sx={{ height: '100%', display: 'flex', alignItems: 'stretch', flexDirection: 'column' }}>
    ...
    <Box sx={{ p: 3, flex: 1, width: '100%' }}><Stack direction="row"   sx={{ mb: 2 , justifyContent: "space-between", alignItems: "center" }}><Eyebrow>{kind === 'graph' ? 'RELATIONSHIPS' : kind === 'manager' ? 'ORCHESTRATION' : 'INTERACTIVE TOOLS'}</Eyebrow><Status value={demo.status} /></Stack>
