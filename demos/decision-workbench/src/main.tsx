...
const engineIds: ModelId[] = ['llm-adapter', 'modernbert', 'gliclass'];
const engineLabels: Record<ModelId, string> = { 'llm-adapter': 'System One Adapter', modernbert: 'ModernBERT', gliclass: 'GLiClass' };
...
function ModelResult({ result, task, name, waiting }: { result?: Result; task: Task; name: string; waiting: boolean }) {
  ...
  return <Panel title={result?.modelName || name} action={<Chip variant="outlined" label={result?.status || (waiting ? 'Waiting' : 'Not run')} color={result?.status === 'ok' ? 'success' : result ? 'warning' : 'default'} />}>
    {!result ? <Box className="result-empty">{waiting && <CircularProgress size={19} />}<Typography variant="body2" color="text.secondary">{waiting ? '判断結果を待っています。ローカルモデルの初回loadや外部APIの応答には時間がかかることがあります。' : '入力と候補を用意して実行すると、このengineの判断が表示されます。'}</Typography></Box> : <>
      ...
      {!!result.scores.length && <Box sx={{ mt: 2.5 }}><Typography variant="body2" sx={{ fontWeight: 650 }}>{reportedProbabilities ? 'LLM-reported probabilities' : 'Candidate scores'}</Typography><Typography variant="caption" color="text.secondary">{reportedProbabilities ? 'LLMが生成した値 / 未校正 · 正解率ではありません' : `${result.scoreKind} · 正解率ではありません`}</Typography><Stack spacing={1.5} sx={{ mt: 1.5 }}>{result.scores.map(score => <Box key={score.optionId}><Stack direction="row" sx={{ justifyContent: 'space-between', gap: 1, mb: .5 }}><Typography variant="body2" sx={{ fontWeight: score.optionId === result.selectedOptionId ? 650 : 400 }}>{optionLabel(task, score.optionId)}</Typography><Typography variant="body2" className="mono">{scoreLabel(score.score)}</Typography></Stack><LinearProgress variant="determinate" value={Math.max(0, Math.min(100, score.score * 100))} color={score.optionId === result.selectedOptionId ? 'primary' : 'secondary'} sx={{ height: 7, borderRadius: 4, bgcolor: '#f0f2f7' }} /></Box>)}</Stack></Box>}
      <Box className="result-timing"><div><Eyebrow>{isAdapter ? 'LOCAL MODEL LOAD' : 'MODEL LOAD'}</Eyebrow><strong>{isAdapter ? '対象外' : elapsed(result.loadMs)}</strong></div><div><Eyebrow>{isAdapter ? result.provider === 'codex' ? 'CODEX TIME' : 'API TIME' : 'INFERENCE'}</Eyebrow><strong>{elapsed(result.inferenceMs)}</strong></div></Box>
      {isAdapter && <Box className="adapter-usage"><Typography variant="caption" color="text.secondary">Input tokens{result.usage?.input_tokens_total !== undefined ? ' (total)' : ''}: {result.usage?.input_tokens_total ?? result.usage?.input_tokens ?? '—'} · Output tokens{result.usage?.output_tokens_total !== undefined ? ' (total)' : ''}: {result.usage?.output_tokens_total ?? result.usage?.output_tokens ?? '—'} · Retries: {result.usage?.n_retries ?? '—'}</Typography></Box>}
      <Accordion disableGutters elevation={0} sx={{ mt: 1 }}><AccordionSummary expandIcon={<span>⌄</span>}><Typography variant="caption">{isAdapter ? 'Decision request / raw output / scores' : 'Prompt / raw output / raw scores'}</Typography></AccordionSummary><AccordionDetails>{result.warning && <Box sx={{ mb: 2 }}><Typography variant="caption" sx={{ fontWeight: 650 }}>Model notes</Typography><Typography variant="body2" color="text.secondary">{result.warning}</Typography></Box>}<Typography variant="caption">{isAdapter ? 'Decision request (SDK input)' : 'Input prompt'}</Typography><pre className="code-block">{result.prompt || '(not available)'}</pre><Typography variant="caption">Raw answer</Typography><pre className="code-block">{raw(result.rawAnswer ?? '(not provided)')}</pre><Typography variant="caption">Raw scores</Typography><pre className="code-block">{JSON.stringify(result.scores, null, 2)}</pre>{isAdapter && <><Typography variant="caption">Execution metadata</Typography><pre className="code-block">{JSON.stringify({ provider: result.provider, providerModel: result.providerModel, requestedModel: result.requestedModel, usage: result.usage }, null, 2)}</pre></>}</AccordionDetails></Accordion>
    </>}
  </Panel>;
}
...
function Workbench() {
  ...
  const [models, setModels] = useState<ModelId[]>(['llm-adapter']);
  ...
  return <LabShell number="04" title="Decision Workbench" subtitle="分類・基準判定・情報の十分性。同じ問いを判断APIやローカルモデルに渡し、入力の変化が判断にどう現れるか観察する。" actions={<Chip label="REAL INFERENCE" variant="outlined" />}>
    ...
    <Box className="workbench-toolbar"><Box><Eyebrow>COMPARE DECISION ENGINES</Eyebrow><Typography variant="body2" color="text.secondary">System One Adapterは、設定したLLMをJevと同じAPI形式で呼び出します。Jevのモデル自体や、速度・校正性能を再現するものではありません。</Typography><Typography variant="caption" color="text.secondary">候補score・LLM生成確率と、参照解への一致は別の指標です。engine間のscoreをそのまま比較しないでください。</Typography></Box><Button variant="outlined" disabled={loading || !!busy} onClick={() => void refreshConfig()}>{loading ? 'Checking setup…' : 'Refresh model setup'}</Button></Box>
