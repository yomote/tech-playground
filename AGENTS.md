...
- Decision Workbench uses direct Jev, optional local models and an explicitly configured LLM through System One Adapter. The adapter is not Jev itself. Never replace unavailable inference with fabricated predictions; distinguish Jev probabilities/confidence, model logits and LLM-reported probabilities, keep scores separate from correctness, and clear fixture expectations when inputs change. Report API round-trip time separately from pure model compute. Keep provider keys server-side and out of responses and run logs.
